...
# --- 프롬프트 정의 ---
# 각 프롬프트는 명확한 작업 지시와 출력 형식을 가집니다.
...
def call_openai_api(prompt: str, model="gpt-3.5-turbo", max_tokens=2048) -> str | None:
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.2, # 일관된 결과를 위해 온도를 낮춤
        )
...
def analyze_network_json(chat_log: str, lang: str = 'ko'):
    """
    채팅 기록을 분석하여 관계 네트워크 데이터를 JSON 형식으로 반환합니다.
    """
    # 프롬프트에 chat_log를 채워넣습니다.
    ...
    try:
        ...
        # JSON 변환에 실패하면, 디버깅을 위해 원본 응답을 포함한 오류를 반환합니다.
        return {"error": "Failed to decode JSON from LLM response.", "raw_response": result_text}

# 참고: analyze_fatigue_json 함수도 동일한 구조로 만들 수 있습니다.
